            ...
            <div class="col-lg-12 mx-auto">
                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <form class="row g-3" @keydown="allErrors.clear($event.target.name)"
                                            ...
                                            <div class="col-lg-6 col-md-6 col-sm-12">
                                                ...
                                                <select name="bill_payer" class="form-control" v-model="form.bill_payer">
                                                    <option value="">{{ translate('Choose') }}</option>
                                                    <option>Owner</option>
                                                    <option>Flat</option>
                                                </select>
                                                <span v-if="this.allErrors.has('bill_payer')"
                                                    class="error text-danger text-bold ms-2 mt-2"
                                                    v-text="this.allErrors.get('bill_payer')">
                                                </span>
                                            </div>
                    ...
                    <div class="card-body" v-if="data.length > 0">
                        ...
                        <div id="print-div">
                            <h5 class="text-center my-1">{{ translate('Maintenance Cost') }} {{
                                translate('Report') }}</h5>
                                ...
                                <table class="table align-middle table-striped">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>{{ translate('Bill') }} {{ translate('Payer') }}</th>
                                    ...
                                    <tbody>
                                        <tr v-for="item in  data " :key="item.id">
                                            ...
                                            <td>
                                                <span>{{ item.bill_payer }}</span><span v-if="item.bill_payer == 'Flat'">({{
                                                    formateFlat(item.checked_flat_names) }})</span>
                                            </td>
                                            <td>
...
export default {
    ...
    methods:
    {
        ...
        filterData() {
            this.loader(true)
            axios
                .post("/report/maintenance", this.form)
                .then((response) => {
                    this.loader(false)
